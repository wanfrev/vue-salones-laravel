...
export function formatBreakdownLabel(
  breakdown: PaymentBreakdownItem[] | null | undefined,
): string {
  if (!breakdown || !Array.isArray(breakdown) || breakdown.length <= 1)
    return "";
  return breakdown
    .map((p) => {
...
export function sumVESBreakdownInputAmounts(
  breakdown: PaymentBreakdownItem[] | null | undefined,
): number {
...
export function getProductSaleCurrency(
  ...
  refId: string | null,
  movementRate: number,
  appointmentPaymentMap: Map<
    string,
    { breakdown: PaymentBreakdownItem[]; exchangeRate: number }
  >,
  ...
  if (refType === "appointment" && refId) {
    ...
    if (paymentEntry) {
      const allVES =
        paymentEntry.breakdown.length > 0 &&
        paymentEntry.breakdown.every((b) => b.currency === "VES");
  ...
  for (const row of rawTransactions) {
    ...
    if (groupId) {
      ...
      if (existing) {
        ...
        if (!existing.employees.includes(employeeName))
...
export function buildProductSalesDetails(
  ...
  return rawInventoryMovements.map((r: any, idx: number) => {
    ...
    const { currency, exchangeRateUsed, originalAmount } =
      getProductSaleCurrency(
        r.reference_type ?? null,
        r.reference_id ?? null,
        Number(r.exchange_rate_used ?? 1),
        appointmentPaymentMap,
        r.notes,
      );
    return {
      id: r.id ?? `product-${idx}`,
      ...
      clientName: r.clients?.full_name || undefined,
      quantity,
      unitPrice,
      total,
      currency,
      exchangeRateUsed,
      originalAmount:
        currency === "VES" ? total * exchangeRateUsed : originalAmount(total),
    };
  });
}
...
export function buildUnifiedTransactions(
  ...
  for (const tx of rawTransactions) {
    ...
    if (groupId) {
      ...
      result.push({
        ...
        description:
          (tx.appointments?.clients?.full_name ?? "—") + " · " + serviceName,
        method: formatBreakdownLabel(breakdown) || formatMethod(tx.method),
        amount: serviceAmt,
        type: "ingreso",
  ...
  for (const [, group] of groupMap) {
    ...
    result.push({
      ...
      description:
        (firstTx.appointments?.clients?.full_name ?? "—") +
        " · " +
        group.services.join(" + "),
      method: formatBreakdownLabel(breakdown) || formatMethod(firstTx.method),
      amount: group.totalAmount,
      type: "ingreso",
  ...
  for (const ep of rawEmployeePayments ?? []) {
    const epCurrency = ((ep as any).currency === "VES" ? "VES" : "USD") as
      | "USD"
      | "VES";
    result.push({
      id: "ep-" + ep.id,
      ...
      _originalAmount:
        epCurrency === "VES"
          ? Number((ep as any).original_amount ?? 0)
          : Number(ep.amount),
    });
  }

  for (const ex of rawExpenses ?? []) {
    const exCurrency = ((ex as any).currency === "VES" ? "VES" : undefined) as
      | "USD"
      | "VES"
      | undefined;
    result.push({
      ...
      _originalAmount:
        exCurrency === "VES"
          ? Number((ex as any).original_amount ?? 0)
          : undefined,
  ...
  for (const row of rawTransactions) {
    ...
    if (assistantId && assistantPct > 0) {
      rows.push({
        id: `${row.id}-asst`,
        employee:
          (row.appointments?.assistant_profile?.full_name ?? "—") +
          " (asistente)",
        client: row.appointments?.clients?.full_name ?? "—",
        service: row.appointments?.services?.name ?? "—",
        amount: row.total_amount,
        percentage: assistantPct,
        earnings: serviceAmount * (assistantPct / 100),
        tipAmount: 0,
      });
    }
  }
