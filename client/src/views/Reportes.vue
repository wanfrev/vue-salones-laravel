<template>
  <div class="h-full max-w-7xl mx-auto space-y-6 flex flex-col">
    ...
    <div class="rounded-2xl border border-border bg-surface shadow-sm overflow-hidden flex-1 flex flex-col min-h-[400px]">
      ...
      <div v-else class="flex-1 overflow-auto">
        <table class="w-full text-left text-sm whitespace-nowrap">
          ...
          <tbody class="divide-y divide-border">
            <tr v-for="report in paginatedReports" :key="report.id" class="transition-colors hover:bg-bg-secondary/50 group">
      ...
      <div v-if="filteredReports.length > 0" class="flex items-center justify-between border-t border-border px-4 py-2.5 shrink-0">
        <div class="text-xs sm:text-sm text-text-muted">
          {{ paginationStart }}-{{ paginationEnd }} de {{ filteredReports.length }}
        </div>
...
const {
  ...
  previousPage,
  pageNumbers,
} = usePagination({ data: filteredReports, pageSize: 20 })
