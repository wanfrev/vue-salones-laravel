<template>
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div class="flex items-center gap-3">
        ...
        <div v-if="activeSaleType === 'retail_only'" class="relative w-full sm:w-72">
          ...
          <div
            v-if="showRetailDropdown && retailFilteredProducts.length > 0"
          ...
          >
            <button
              v-for="product in retailFilteredProducts"
              :key="product.id"
        ...
        <!-- Normal mode: "+ Venta directa" button -->
        <button
          v-else
          @click="startRetailOnly"
          class="flex items-center gap-1.5 rounded-lg border border-primary/30 px-3 py-2 text-xs font-semibold text-primary shadow-sm transition-all duration-200 hover:bg-primary/5"
        >
  ...
  <!-- ================================================================ -->
  <!-- UNIFIED LAYOUT: Left (appointments + inline products) / Right (checkout) -->
  <!-- ================================================================ -->
  <div class="grid grid-cols-1 gap-4 lg:grid-cols-[3fr_2fr]">
    <!-- LEFT PANEL -->
    <div class="space-y-3">
      <!-- Search bar (outside sections, shared) -->
      ...
      <!-- Overdue appointments: "Citas realizadas" -->
      <div v-if="overdueAppointments.length > 0" class="rounded-xl border border-border bg-surface p-4 mb-3">
        ...
        <div class="space-y-1.5">
          <template v-for="appt in overdueAppointments" :key="appt.id">
            <button
              @click="selectAppointment(appt)"
              :class="[
                'w-full text-left rounded-lg border p-2.5 transition-theme',
                selectedId === appt.id ? 'border-primary bg-primary/5 ring-1 ring-primary/20' : 'border-border hover:border-border-strong hover:bg-bg-secondary'
              ]"
            >
            ...
            <button
              type="button"
              @click="goToAppointmentInCalendar(appt)"
            ...
            <div v-if="selectedId === appt.id" class="ml-2 mr-1 mb-1.5 rounded-lg border border-primary/20 bg-primary/5 p-2">
              <div class="relative">
                <input
                  v-model="inlineProductSearch"
                  type="text"
                ...
                >
                  <button
                    v-for="product in inlineFilteredProducts"
                    :key="product.id"
                    @mousedown.prevent="addInlineProduct(product)"
                    :disabled="Number(product.available_qty ?? 0) <= 0"
      ...
      <!-- Upcoming appointments: "Citas pendientes" -->
      <div v-if="upcomingAppointments.length > 0" class="rounded-xl border border-border bg-surface p-4">
        ...
        <div class="space-y-1.5">
          <template v-for="appt in upcomingAppointments" :key="appt.id">
            <button
              @click="selectAppointment(appt)"
              :class="[
                'w-full text-left rounded-lg border p-2.5 transition-theme',
            ...
            <!-- Product search inline below selected appointment -->
            <div v-if="selectedId === appt.id" class="ml-2 mr-1 mb-1.5 rounded-lg border border-primary/20 bg-primary/5 p-2">
              <div class="relative">
                <input
                  v-model="inlineProductSearch"
    ...
    <div class="space-y-4 lg:sticky lg:top-20 h-fit">
      <POSPaymentPanel
        ...
        :other-currency="paymentCtx.otherCurrency.value"
        ...
        :tip-participants="tipParticipants"
        :tip-allocations="tipAllocations"
        :tip-allocated-total="tipAllocatedTotal"
        :tip-remaining="tipRemaining"
        :show-tip-adjust="showTipAdjust"
...
const filteredAppointments = computed(() => {
  ...
  return appointments.value.filter((a: any) => {
    ...
    const service = normalize(a.services?.name ?? '')
    return name.startsWith(q) || phone.startsWith(rawQ) || service.startsWith(q)
  })
})
...
const overdueAppointments = computed(() =>
  filteredAppointments.value.filter(a => new Date(a.start_time) <= now.value)
)
...
// Product filtering for each search context
const filterProducts = (query: string): any[] => {
  ...
  return (products.value as any[]).filter((p: any) => p.name.toLowerCase().includes(q)).slice(0, 8)
}

const retailFilteredProducts = computed(() => filterProducts(retailProductSearch.value))
const inlineFilteredProducts = computed(() => filterProducts(inlineProductSearch.value))

const addRetailProduct = (product: any) => {
  cartCtx.addProduct(product)
  retailProductSearch.value = ''
  showRetailDropdown.value = false
}

const addInlineProduct = (product: any) => {
  cartCtx.addProduct(product)
  inlineProductSearch.value = ''
  showInlineDropdown.value = false
}

const onRetailBlur = () => {
  setTimeout(() => { showRetailDropdown.value = false }, 150)
}
...
const servicePrice = computed(() => {
  ...
  // Para citas con múltiples servicios, siempre sumar los precios de los miembros
  if (Array.isArray(appt.members) && appt.members.length > 0) {
    return appt.members.reduce((sum: number, m: any) => sum + Number(m.price ?? 0), 0)
  }
  
  // Fallback para citas individuales
...
const retailTotal = computed(() => cartCtx.productsTotal.value)

const grandTotal = computed(() => {
  if (activeSaleType.value === 'retail_only') return retailTotal.value
  return servicePrice.value + cartCtx.productsTotal.value + paymentCtx.tipAmount.value
})
...
const tipParticipants = computed<TipParticipant[]>(() => {
  ...
  if (appt.isGroup && Array.isArray(appt.members)) {
    ...
    for (const m of appt.members as Array<{ employeeId?: string; employeeName?: string }>) {
      if (!m.employeeId) continue
      if (!map.has(m.employeeId)) map.set(m.employeeId, m.employeeName ?? 'Empleado')
    }
    return Array.from(map.entries()).map(([employeeId, employeeName]) => ({ employeeId, employeeName }))
  }

  const result: TipParticipant[] = []
  if (appt.employee_id) {
    result.push({ employeeId: appt.employee_id, employeeName: appt.profiles?.full_name ?? 'Empleado' })
  }
  if (appt.assistant_employee_id) {
    result.push({
      employeeId: appt.assistant_employee_id,
      employeeName: appt.assistant_profile?.full_name ?? 'Asistente',
    })
  }

  const unique = new Map<string, TipParticipant>()
  for (const p of result) unique.set(p.employeeId, p)
  return Array.from(unique.values())
})

const distributeEqualTip = (totalTip: number, participants: TipParticipant[]): Record<string, number> => {
  if (participants.length === 0 || totalTip <= 0) return {}
  const cents = Math.round(totalTip * 100)
  ...
  participants.forEach((p, idx) => {
    ...
    next[p.employeeId] = shareCents / 100
  })
  return next
}

const setEqualTipAllocation = () => {
  tipAllocations.value = distributeEqualTip(paymentCtx.tipAmount.value, tipParticipants.value)
  tipManual.value = false
}

const tipAllocatedTotal = computed(() => {
  return tipParticipants.value.reduce((sum, p) => sum + Number(tipAllocations.value[p.employeeId] ?? 0), 0)
})

const tipRemaining = computed(() => Number((paymentCtx.tipAmount.value - tipAllocatedTotal.value).toFixed(2)))

const normalizedTipAllocations = computed<TipAllocationItem[]>(() => {
  return tipParticipants.value
    .map(p => ({
      employee_id: p.employeeId,
      employee_name: p.employeeName,
      amount: Number((tipAllocations.value[p.employeeId] ?? 0).toFixed(2)),
    }))
    .filter(x => x.amount > 0)
})

watch([tipParticipants, () => selectedId.value], () => {
  setEqualTipAllocation()
  showTipAdjust.value = false
}, { immediate: true })

watch(() => paymentCtx.tipAmount.value, () => {
  if (!tipManual.value) {
    setEqualTipAllocation()
  }
})
...
const canPay = computed(() => {
  ...
  if (activeSaleType.value !== 'retail_only' && paymentCtx.tipAmount.value > 0 && tipParticipants.value.length > 0) {
    if (Math.abs(tipRemaining.value) >= 0.01) return false
  }
...
const selectAppointment = (appt: any) => {
  if (selectedAppointment.value?.id === appt.id) {
    selectedAppointment.value = null
    cartCtx.clearCart()
    paymentCtx.reset()
    return
  }
  const wasRetail = activeSaleType.value === 'retail_only'
...
const goToAppointmentInCalendar = (appt: any) => {
  const fecha = new Date(appt.start_time).toISOString().slice(0, 10)
  router.push({ name: 'admin-calendario', query: { fecha } })
}
...
const setTipAllocation = (employeeId: string, value: number) => {
  tipManual.value = true
  tipAllocations.value = {
    ...tipAllocations.value,
...
const handleRetailPayment = async () => {
  ...
  try {
    const paymentMethod = paymentCtx.paymentMethod.value
    const currency = paymentCtx.paymentMethods.find(m => m.value === paymentMethod)?.currency ?? paymentCtx.otherCurrency.value
    const rate = exchangeRate.value

    for (const item of cartCtx.cart.value) {
      const saleCurrency = paymentMethod === 'other'
        ? paymentCtx.otherCurrency.value
        : (currency as 'USD' | 'VES') ?? 'USD'

      await sellProduct(
        businessId.value!,
        item.productId,
        item.quantity,
        paymentCtx.paymentNotes.value || `POS Venta directa · ${paymentMethod}`,
        null,
        item.unitPrice,
        rate,
        saleCurrency,
    ...
    const msg = (err as any)?.message ?? 'Error al procesar la venta'
    showError(msg)
  } finally {
    retailProcessing.value = false
  }
}

const handleGroupPayment = async (appt: any) => {
  const groupIds: string[] = appt.groupIds
  const members: Array<{ appointmentId: string; employeeId: string; serviceName: string; employeeName: string; price: number }> = appt.members ?? []
  const groupPrice: number = appt.groupPrice
  const products = cartCtx.cart.value
  const method = paymentCtx.paymentMethod.value
  const serviceTotal = servicePrice.value
  const productsTotal = cartCtx.productsTotal.value

  if (groupPrice <= 0 || members.length === 0) {
    showError('El precio del grupo no es válido')
    return
  }

  paymentCtx.isProcessing.value = true
  try {
    const exchangeRt = exchangeRate.value
    const notes = paymentCtx.paymentNotes.value
    const breakdownSource = paymentCtx.paymentsBreakdown.value

    let methodBreakdowns: Array<{ method: string; inputAmount: number; currency: string; amount: number }>
    if (method !== 'mixed') {
      const pMethodObj = paymentCtx.paymentMethods.find(m => m.value === method)
      const currency = (pMethodObj?.currency as string) ?? paymentCtx.otherCurrency.value
      const inputAmount = currency === 'VES' ? grandTotal.value * exchangeRt : grandTotal.value
      methodBreakdowns = [{ method, inputAmount, currency, amount: grandTotal.value }]
    } else {
      methodBreakdowns = breakdownSource.map(item => ({
        ...item,
        amount: item.currency === 'VES' ? item.inputAmount / exchangeRt : item.inputAmount,
      }))
    }
    ...
    for (const m of members) {
      if (m.employeeId) {
        employeeTotalPrice.set(m.employeeId, (employeeTotalPrice.get(m.employeeId) ?? 0) + m.price)
      }
    }
    for (let i = 0; i < groupIds.length; i++) {
      ...
      if (isLast) {
        serviceShare = Math.max(0, Math.round(remainingService * 100) / 100)
      } else {
        serviceShare = Math.round(serviceTotal * proportion * 100) / 100
        remainingService -= serviceShare
      }
      ...
      const tipProportion = employeeId && fullTip > 0
        ? members[i].price / (employeeTotalPrice.get(employeeId) ?? 1)
        : 0
      const memberTip = Number((fullTip * tipProportion).toFixed(2))

      const memberBreakdown = methodBreakdowns.map(b => ({
        ...b,
        inputAmount: Math.round(b.inputAmount * proportion * 100) / 100,
        amount: Math.round(b.amount * proportion * 100) / 100,
      }))

      const memberTipAllocations = employeeId && memberTip > 0
        ? [{ employee_id: employeeId, employee_name: members[i]?.employeeName ?? 'Empleado', amount: memberTip }]
        : []
      const memberBreakdownWithTips = memberTipAllocations.length > 0
        ? [{ ...(memberBreakdown[0] as any), tip_allocations: memberTipAllocations }, ...memberBreakdown.slice(1)]
        : memberBreakdown
      ...
      if (i === 0) {
        await recordSale({
          ...
          paymentsBreakdown: memberBreakdownWithTips as any,
          businessId: businessId.value!,
          branchId: branchId.value,
          tipAmount: memberTip,
        })
      } else {
        await recordPaymentOnly({
          appointmentId: groupIds[i],
          amount,
          method: method as any,
          notes,
    ...
    showSuccess(`Cobro de ${formatDual(grandTotal.value)} registrado correctamente`)
    selectedAppointment.value = null
    cartCtx.clearCart()
    paymentCtx.reset()
  } catch (err: any) {
    const message = err?.message ?? 'Error al procesar el pago del grupo'
    showError(message)
    console.error('[handleGroupPayment]', err)
  } finally {
    paymentCtx.isProcessing.value = false
  }
}
...
const confirmPayment = async () => {
  ...
  const ok = await paymentCtx.processPayment(
    ...
    exchangeRate.value,
    formatDual,
    normalizedTipAllocations.value,
  )
  if (ok) {
    selectedAppointment.value = null
    cartCtx.clearCart()
