            ...
            <tr class="text-sm transition-theme hover:bg-bg-secondary/60 cursor-pointer" @click="toggleExpand(inv.id)">
              ...
              <td class="py-3 text-center">
                <div class="flex items-center justify-center gap-1">
                  ...
                  <button @click.stop="summaryCtx.startEdit(inv)" class="rounded-md bg-primary/10 p-1.5 text-primary transition-theme hover:bg-primary/20" title="Editar">
        ...
        <template v-if="isTienda">
          <div v-for="inv in items" :key="inv.id" class="rounded-lg border border-border-subtle bg-bg-secondary/30 p-3 space-y-2 text-sm cursor-pointer hover:border-primary/40" @click="toggleExpand(inv.id)">
            ...
            <div class="flex items-center gap-1">
              <button @click.stop="summaryCtx.startEdit(inv)" class="flex items-center justify-center gap-1 flex-1 rounded-lg py-1.5 text-xs text-primary hover:bg-primary/10 transition-colors mt-1 border border-primary/10">
              ...
              <button @click.stop="summaryCtx.confirmDeleteTransaction(inv.id)" class="flex items-center justify-center gap-1 flex-1 rounded-lg py-1.5 text-xs text-danger hover:bg-danger/10 transition-colors mt-1 border border-danger/10">
            ...
            <div v-if="expandedIds.has(inv.id)" class="mt-2 pt-2 border-t border-border-subtle space-y-1.5 bg-surface/50 p-2 rounded-lg">
              <div v-for="item in inv.items" :key="item.id" class="flex items-center justify-between text-xs py-1 border-b border-border-subtle/30 last:border-0">
                ...
                <div class="flex items-center gap-2 text-right">
                  <span class="text-text-muted">{{ item.quantity }} x {{ formatUSD(item.unitPrice) }}</span>
                  <span class="font-semibold text-success tabular-nums">{{ formatUSD(item.total) }}</span>
                  <button @click.stop="summaryCtx.handleDeleteProductSale(item.id, item.product)" class="flex h-5 w-5 shrink-0 items-center justify-center rounded text-text-muted hover:bg-danger/10 hover:text-danger transition-colors" title="Eliminar producto de factura">
                    <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                  </button>
                </div>
              </div>
      ...
      <template #desktop-tbody="{ items }">
        <template v-for="tx in items" :key="tx.id">
          ...
          >
            ...
            <td class="py-3"><span :class="['rounded-full px-2 py-0.5 text-xs font-semibold', tx.type === 'ingreso' ? 'bg-success/10 text-success' : tx.type === 'nomina' ? 'bg-warning/10 text-warning' : 'bg-danger/10 text-danger']">{{ tx.type === 'ingreso' ? (tx.items ? (tx.receiptCode ? 'Factura #' + tx.receiptCode : 'Factura') : tx.source === 'product_sale' ? (tx.receiptCode ? 'Venta #' + tx.receiptCode : 'Venta') : 'Ingreso') : tx.type === 'nomina' ? 'Nomina' : 'Gasto' }}</span></td>
