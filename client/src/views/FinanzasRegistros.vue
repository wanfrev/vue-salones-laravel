<template>
  <section class="space-y-4">
    ...
    <div v-else-if="tipo === 'pagos'" class="space-y-4">
      ...
      <div class="rounded-xl border border-border bg-surface p-4">
        ...
        <div class="overflow-x-auto">
          ...
          <tbody class="divide-y divide-border-subtle">
            <tr v-for="ep in paymentsCtx.paymentsMade.value" :key="ep.id" class="text-sm transition-theme hover:bg-bg-secondary/50"><td class="py-3 text-text-secondary whitespace-nowrap">{{ ep.paymentDate }}</td><td class="py-3 font-medium text-text">{{ ep.employeeName }}</td><td class="py-3 text-text-secondary">{{ formatMethod(ep.paymentMethod) }}</td><td class="py-3 text-right"><div class="font-medium text-danger">{{ ep.currency === 'VES' ? formatVESEs(ep.originalAmount) : formatUSD(ep.amount) }}</div><div class="text-xs text-text-muted">{{ ep.currency === 'VES' ? formatUSD(ep.amount) : formatVESInline(ep.amount, ep.exchangeRateUsed) + ' Bs' }}</div></td>
    ...
    <RecordSection v-else-if="tipo === 'ventas-productos'" title="Ventas de productos del periodo" :items="paginatedVentas" :total-count="summaryCtx.productSalesDetails.value.length" empty-message="No hay ventas de productos en este periodo." :pages="ventasPages" :page-size="pageSize" @prev="prevPage" @next="nextPage">
      <template #desktop-thead><th class="pb-3 text-left text-xs font-semibold uppercase text-text-muted">Fecha</th><th class="pb-3 text-left text-xs font-semibold uppercase text-text-muted hidden sm:table-cell">Cliente</th><th class="pb-3 text-left text-xs font-semibold uppercase text-text-muted">Producto</th><th class="pb-3 text-right text-xs font-semibold uppercase text-text-muted">Cantidad</th><th class="pb-3 text-right text-xs font-semibold uppercase text-text-muted">Precio unit.</th><th class="pb-3 text-right text-xs font-semibold uppercase text-text-muted">Total</th><th class="pb-3 text-center text-xs font-semibold uppercase text-text-muted w-10"></th></template>
      <template #desktop-tbody="{ items }">
        <tr v-for="row in items" :key="row.id" class="text-sm transition-theme hover:bg-bg-secondary/50"><td class="py-3 text-text-secondary whitespace-nowrap">{{ row.date }}</td><td class="py-3 text-text-secondary hidden sm:table-cell">{{ row.clientName || '—' }}</td><td class="py-3 font-medium text-text">{{ row.product }}</td><td class="py-3 text-right text-text">{{ row.quantity }}</td><td class="py-3 text-right text-text whitespace-nowrap">{{ formatUSD(row.unitPrice) }}</td><td class="py-3 text-right font-medium text-success whitespace-nowrap"><div>{{ row.currency === 'VES' ? formatVESEs(row.originalAmount) : formatUSD(row.total) }}</div><div class="text-[10px] text-text-muted mt-0.5">{{ row.currency === 'VES' ? formatUSD(row.total) : formatVESInline(row.total, row.exchangeRateUsed) + ' Bs' }}</div></td>
      ...
      <template #mobile-cards="{ items }">
        <div v-for="row in items" :key="row.id" class="rounded-lg border border-border-subtle bg-bg-secondary/30 p-3 space-y-2 text-sm">
          ...
          <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs"><span class="text-text-muted">Cantidad</span><span class="text-text text-right">{{ row.quantity }}</span><span class="text-text-muted">Precio unit.</span><span class="text-text text-right">{{ formatUSD(row.unitPrice) }}</span></div>
    ...
    <RecordSection v-else title="Transacciones del periodo" :items="paginatedTransacciones" :total-count="summaryCtx.allTransactions.value.length" empty-message="No hay transacciones en este periodo." :pages="transaccionesPages" :page-size="pageSize" @prev="prevPage" @next="nextPage">
      ...
      <template #desktop-tbody="{ items }">
        <tr v-for="tx in items" :key="tx.id" class="text-sm transition-theme hover:bg-bg-secondary/50"><td class="py-3 text-text-secondary whitespace-nowrap">{{ tx.date }}</td><td class="py-3 font-medium text-text"><div>{{ tx.description }}</div><div class="text-[11px] text-text-muted mt-0.5">{{ tx.sourceLabel }}</div></td><td class="py-3"><span :class="['rounded-full px-2 py-0.5 text-xs font-semibold', tx.type === 'ingreso' ? 'bg-success/10 text-success' : tx.type === 'nomina' ? 'bg-warning/10 text-warning' : 'bg-danger/10 text-danger']">{{ tx.type === 'ingreso' ? (tx.source === 'product_sale' ? 'Venta' : 'Ingreso') : tx.type === 'nomina' ? 'Nomina' : 'Gasto' }}</span></td><td class="py-3 text-text-secondary">{{ tx.method }}</td><td class="py-3 text-right" :class="tx.type === 'ingreso' ? 'text-success' : 'text-danger'"><div class="font-medium">{{ tx.type === 'ingreso' ? '' : '-' }}{{ tx._currency === 'VES' ? formatVESEs(tx._originalAmount ?? tx.amount) : formatUSD(tx.amount) }}</div><div class="text-[10px] text-text-muted mt-0.5">{{ tx._currency === 'VES' ? formatUSD(tx.amount) : formatVESInline(tx.amount, tx.exchangeRateUsed) + ' Bs' }}</div></td>
