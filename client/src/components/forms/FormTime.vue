  ...
  <div class="space-y-1.5 min-w-0 w-full">
    ...
    <div class="flex items-center gap-1.5 min-w-0 w-full">
      <select :id="inputId" :value="hour12" @change="onHourChange" :disabled="disabled"
        class="flex-1 min-w-[56px] rounded-xl border bg-surface-elevated text-text outline-none transition-theme focus:border-primary focus:ring-2 focus:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-bg-secondary px-2 py-2 text-sm font-medium"
        :class="error ? 'border-danger' : 'border-border hover:border-border-strong'">
        <option v-for="h in 12" :key="h" :value="h">{{ h }}</option>
      </select>
      <span class="text-text-muted font-bold shrink-0 px-0.5">:</span>
      ...
      <button type="button" :disabled="disabled" @click="toggleAmPm"
        class="shrink-0 rounded-xl border bg-surface-elevated text-xs font-bold px-2 py-2 outline-none transition-theme hover:bg-bg-secondary disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-bg-secondary min-w-[42px] text-center"
        :class="[
