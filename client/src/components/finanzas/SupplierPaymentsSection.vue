<template>
  ...
  <Teleport to="body">
    <div v-if="ctx.showModal.value" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4"
      ...
      <div class="w-full max-w-md rounded-2xl border border-border bg-surface p-6 shadow-xl">
        ...
        <form class="space-y-4" @submit.prevent="handleSave">
          <div>
            ...
            <p v-if="ctx.form.value.supplierId && ctx.supplierMap.value[ctx.form.value.supplierId]"
              class="mt-1.5 text-xs text-text-muted">
              Deuda total: <span class="font-medium text-text">{{
                formatUSD(ctx.supplierMap.value[ctx.form.value.supplierId].totalDebt) }}</span>
              <span class="mx-1">|</span>
              Pendiente: <span class="font-medium"
              ...
              <template v-if="ctx.selectedSupplierPendingAfter.value > 0 && ctx.form.value.amount > 0">
                <span class="mx-1">|</span>
                Restaría: <span class="font-medium text-text-muted">{{ formatUSD(ctx.selectedSupplierPendingAfter.value)
                  }}</span>
              </template>
            </p>
          </div>
