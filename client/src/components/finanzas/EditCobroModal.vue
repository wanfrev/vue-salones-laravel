      ...
      <div
        ...
        <div class="mb-4">
          ...
          <p class="text-sm text-text-muted" v-if="summaryCtx.editingTransaction.value">
            {{ summaryCtx.editingTransaction.value.client }} · {{ summaryCtx.editingTransaction.value.service }} · {{
              summaryCtx.editingTransaction.value.date }}
          </p>
        </div>
        <form class="space-y-4" @submit.prevent="summaryCtx.saveEdit">
          <div>
            ...
            <select :value="summaryCtx.editingMethod.value"
              @change="summaryCtx.setEditingMethod(($event.target as HTMLSelectElement).value as any)"
              class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text outline-none transition-theme focus:border-primary focus:ring-2 focus:ring-primary/30">
              <option v-for="opt in summaryCtx.paymentMethodOptions" :key="opt.value" :value="opt.value">{{ opt.label }}
              </option>
            </select>
          </div>
          <div>
            <label class="mb-1 block text-sm font-medium text-text">Moneda</label>
            <select :value="summaryCtx.editingCurrency.value"
          ...
          <div v-if="summaryCtx.isEditingMixed.value"
            ...
            <div v-for="(breakItem, bidx) in summaryCtx.editingBreakdown.value" :key="bidx"
              ...
              <select :value="breakItem.method"
                ...
                <option v-for="opt in summaryCtx.paymentMethodOptions.filter((o: any) => o.value !== 'mixed')"
              ...
              <span class="text-xs font-medium text-text-muted w-8 text-center">{{ summaryCtx.editingCurrency.value
                }}</span>
              <button v-if="summaryCtx.editingBreakdown.value.length > 1" type="button"
                @click="summaryCtx.removeBreakdownItem(bidx)"
          ...
          <div>
            ...
            <input v-if="!summaryCtx.isEditingMixed.value" v-model.number="summaryCtx.editingAmount.value" type="number"
              min="0.01" step="0.01"
              class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text outline-none transition-theme focus:border-primary focus:ring-2 focus:ring-primary/30"
              placeholder="0.00" required />
            <div v-else class="rounded-lg border border-border bg-bg-secondary px-3 py-2 text-lg font-bold text-text">{{
              formatUSD(summaryCtx.editingTotalAmount.value) }}</div>
          </div>
          <div>
          ...
          <div class="flex items-center justify-end gap-3 pt-1">
            ...
            <button type="submit" :disabled="summaryCtx.editTransactionMutation.isPending.value"
