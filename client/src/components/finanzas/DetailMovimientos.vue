...
<template>
  <div class="mb-4 rounded-xl border border-border bg-surface shadow-sm">
    ...
    <div class="mx-3 sm:mx-5 mt-3 sm:mt-4 mb-0 rounded-xl border border-border-subtle p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3" :class="[
      ...
      <div class="flex items-center gap-3">
        <div class="p-2.5 rounded-lg border shrink-0" :class="[
          ...
          <svg v-if="activeDetailTab === 'cobros'" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          <svg v-else-if="activeDetailTab === 'ventas'" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
          <svg v-else-if="activeDetailTab === 'servicios'" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>
          <svg v-else class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>
        </div>
        <div>
      ...
      <div class="flex items-center gap-2 self-end sm:self-auto">
        <button v-if="activeDetailTab === 'gastos'"
          ...
          <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 4v16m8-8H4" /></svg>
          <span class="hidden sm:inline">Registrar gasto</span><span class="sm:hidden">+ Gasto</span>
        </button>
        <button v-if="activeDetailTab === 'servicios'"
        ...
        <button v-if="canViewDetailTab" type="button"
          ...
          @click="goToDetailTab">
          Ver todos <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 5l7 7-7 7" /></svg>
        </button>
      </div>
    </div>
    ...
    <div class="p-3 sm:p-5">
      ...
      <div v-if="activeDetailTab === 'cobros'">
          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-border-subtle">
              <tr v-for="item in filteredCobrosRows.slice(0, canViewDetailTab ? 5 : Infinity)" :key="item.id" class="text-xs transition-theme hover:bg-bg-secondary/40">
                ...
                <td class="px-3 py-3"><span class="inline-flex items-center rounded-md bg-bg-secondary px-2 py-0.5 text-[11px] font-medium text-text-secondary">{{ item.breakdownLabel || item.method }}</span></td>
                ...
                <td class="px-3 py-3 text-center">
                  <div class="flex items-center justify-center gap-1">
                    ...
                    <button @click="summaryCtx.confirmDeleteTransaction(item.id)" :disabled="summaryCtx.editTransactionMutation.isPending.value || summaryCtx.deleteTransactionMutation.isPending.value" class="rounded-lg p-1.5 text-text-muted transition-theme hover:bg-danger/10 hover:text-danger" title="Eliminar cobro">
                      <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                    </button>
                  </div>
                </td>
              </tr>
        ...
        <div v-else class="flex flex-col items-center justify-center py-12 text-center">
          <div class="flex h-10 w-10 items-center justify-center rounded-full bg-bg-secondary mb-2">
            <svg class="h-5 w-5 text-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          </div>
      ...
      <div v-else-if="activeDetailTab === 'ventas'">
          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-border-subtle">
              <tr v-for="row in allVentasRows.slice(0, canViewDetailTab ? 5 : Infinity)" :key="row.id" class="text-xs transition-theme hover:bg-bg-secondary/40">
                ...
                <td class="px-3 py-3 text-center">
                  <button @click="summaryCtx.handleDeleteProductSale(row.id, row.product)" class="flex h-6 w-6 items-center justify-center rounded text-text-muted hover:bg-danger/10 hover:text-danger transition-colors" title="Eliminar venta">
        ...
        <div v-else class="flex flex-col items-center justify-center py-12 text-center">
          <div class="flex h-10 w-10 items-center justify-center rounded-full bg-bg-secondary mb-2">
            <svg class="h-5 w-5 text-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
          </div>
        ...
        <div v-if="productSalesBreakdown.length" class="mt-4 rounded-lg bg-bg-secondary p-3">
          ...
          <div class="space-y-1">
            <div v-for="(p, idx) in productSalesBreakdown.slice(0,5)" :key="p.name" class="flex items-center justify-between text-xs">
              <span class="text-text-secondary truncate"><span class="font-medium text-text-muted">{{ idx + 1 }}.</span> {{ p.name }}</span>
              ...
              <tr v-for="svc in filteredServicios" :key="svc.id" class="group text-xs transition-theme hover:bg-bg-secondary/40">
                ...
                <td class="px-3 py-3 text-text-secondary hidden sm:table-cell">
                  <div class="flex items-center gap-2">
                    <span class="rounded-full px-2 py-0.5 text-[11px] font-medium bg-primary/10 text-primary">{{ svc.category }}</span>
                    <button @click="openRenameCategoryModal(svc.category)" class="rounded p-0.5 text-text-muted hover:text-primary hover:bg-primary/10 transition-all" title="Editar categoría">
                    ...
                    <button v-if="svcCategories.length > 2" @click="openDeleteCategoryModal(svc.category)" class="rounded p-0.5 text-text-muted hover:text-danger hover:bg-danger/10 transition-all" title="Eliminar categoría">
                ...
                <td class="px-3 py-3 text-right"><div class="font-semibold text-success tabular-nums">{{ formatUSD(svc.price) }}</div><div class="text-[10px] text-text-muted">{{ formatVESInline(svc.price) }} Bs</div></td>
                <td class="px-3 py-3 text-right text-text-secondary hidden sm:table-cell">{{ svc.duration }} min</td>
                <td class="px-3 py-3 text-center"><span :class="['rounded-full px-2 py-0.5 text-[11px] font-medium', svc.status === 'Activo' ? 'bg-success/10 text-success' : 'bg-text-muted/10 text-text-muted']">{{ svc.status }}</span></td>
                <td class="px-3 py-3 text-center">
                  <div class="flex items-center justify-center gap-1">
      ...
      <div v-else>
        ...
        <div v-else-if="expensesError" class="flex flex-col items-center justify-center py-12 text-center">
          <div class="flex h-10 w-10 items-center justify-center rounded-full bg-danger/10 mb-2">
            <svg class="h-5 w-5 text-danger" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" /></svg>
          </div>
        ...
        <div v-else-if="allGastosRows.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          <div class="flex h-10 w-10 items-center justify-center rounded-full bg-bg-secondary mb-2"><svg class="h-5 w-5 text-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M20 12H4" /></svg></div>
          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-border-subtle">
              <tr v-for="expense in allGastosRows.slice(0, canViewDetailTab ? 5 : Infinity)" :key="expense.id" class="text-xs transition-theme hover:bg-bg-secondary/40">
                ...
                <td class="px-3 py-3"><span :class="['rounded-full px-2 py-0.5 text-[11px] font-medium', expense.category === 'Fijos' ? 'bg-info/10 text-info' : expense.category === 'Insumos' ? 'bg-warning/10 text-warning' : 'bg-primary/10 text-primary']">{{ expense.category }}</span></td>
                <td class="px-3 py-3 text-right"><div class="font-medium text-text whitespace-nowrap">{{ expense.currency === 'VES' ? formatVESEs(expense.originalAmount) : formatUSD(expense.amount) }}</div><div class="text-[11px] text-text-muted whitespace-nowrap">{{ expense.currency === 'VES' ? formatUSD(expense.amount) : formatVESInline(expense.amount, expense.exchangeRateUsed) + ' Bs' }}</div></td>
                <td class="px-3 py-3 text-center">
                  <div class="flex items-center justify-center gap-1">
  ...
  <!-- Servicios Modals -->
  <ServicioFormModal ref="servicioModalRef" :is-saving="saveServicioMutation.isPending.value ?? false" @save="handleSaveServicio" />

  <ModalBase :is-open="isDeleteServicioOpen" title="Eliminar servicio" subtitle="Esta acción no se puede deshacer" icon="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" variant="danger" size="sm" confirm-text="Eliminar" cancel-text="Cancelar" :loading="deleteServicioMutation?.isPending.value ?? false" @close="isDeleteServicioOpen = false" @confirm="confirmDeleteServicio" @cancel="isDeleteServicioOpen = false">
    <p class="text-sm text-text-secondary">¿Estás seguro de que deseas eliminar <strong>{{ servicioToDelete?.name }}</strong>? Este servicio será eliminado permanentemente del catálogo.</p>
  </ModalBase>

  <ModalBase :is-open="isRenameCategoryOpen" title="Editar categoría" subtitle="Actualiza el nombre de la categoría" icon="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" size="sm" confirm-text="Guardar" cancel-text="Cancelar" :loading="isUpdatingCategory" @close="closeRenameCategoryModal" @confirm="confirmRenameCategory" @cancel="closeRenameCategoryModal">
    <label class="mb-2 block text-sm font-medium text-text" for="new-cat-name">Nombre</label>
    <input id="new-cat-name" v-model="newCategoryName" type="text" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text outline-none transition-theme focus:border-primary" required />
  </ModalBase>

  <ModalBase :is-open="isDeleteCategoryOpen" title="Eliminar categoría" subtitle="Reasigna los servicios a otra categoría antes de eliminar" icon="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" variant="danger" size="sm" confirm-text="Eliminar categoría" cancel-text="Cancelar" :loading="isUpdatingCategory" @close="closeDeleteCategoryModal" @confirm="confirmDeleteCategory" @cancel="closeDeleteCategoryModal">
    <p class="text-sm text-text-secondary mb-3">Los servicios de <strong>{{ categoryToDeleteName }}</strong> se moverán a:</p>
    <select v-model="replacementCategory" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text outline-none transition-theme focus:border-primary">
