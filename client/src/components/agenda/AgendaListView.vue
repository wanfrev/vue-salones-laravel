<template>
  <div>
    <!-- Error state -->
    <div v-if="error" class="rounded-xl border border-danger/30 bg-danger/5 p-4 mb-4">
      <div class="flex items-start gap-3">
        ...
        <div>
          <p class="text-sm font-semibold text-danger">Error al cargar citas</p>
          <p class="text-xs text-danger/70 mt-0.5">{{ typeof error === 'object' && error !== null ? ((error as any).message || String(error)) : String(error) }}</p>
        </div>
      </div>
    </div>
