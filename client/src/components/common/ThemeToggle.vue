<template>
  <div class="relative">
    ...
    <button @click="toggle"
      ...
      <!-- Sol (modo claro) -->
      <svg v-if="isLight" class="h-5 w-5 text-warning" fill="none" viewBox="0 0 24 24" stroke="currentColor"
        stroke-width="2">
      ...
      <!-- Luna (modo oscuro) -->
      <svg v-else-if="isDark" class="h-5 w-5 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor"
        stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div v-if="showOptions" v-click-outside="closeOptions"
      ...
      <button v-for="option in themeOptions" :key="option.value" @click="selectMode(option.value)"
        ...
        <svg v-if="mode === option.value" class="ml-auto h-4 w-4 text-primary" fill="none" viewBox="0 0 24 24"
          stroke="currentColor">
