      ...
      <div class="rounded-xl border border-border bg-surface p-4 sm:p-6">
        ...
        <div class="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p v-if="payInfo" class="text-xs text-text-muted">
              {{ payInfo.typeLabel }}
        ...
        <div class="rounded-lg bg-bg-secondary/50 border border-border px-3 py-1.5 mb-4 text-center">
    ...
    <template #footer>
      <div class="flex justify-end gap-3 w-full">
        ...
        <button
          type="button"
          @click="triggerPrint"
          class="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-6 py-2.5 text-sm font-semibold text-text-inverse transition-theme hover:bg-primary-hover shadow-sm hover:shadow active:scale-95"
        >
          <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
import EmployeeEarningsCards from '../../views/employee/EmployeeEarningsCards.vue'
...
const payInfo = computed(() => {
  const member = props.employee
  if (!member) return null
  const type = member.payType ?? member.pay_type ?? 'percentage'
  const percentage = Number(member.payPercentage ?? member.pay_percentage ?? 50)
  ...
  const freq = member.salaryFrequency ?? member.salary_frequency ?? 'monthly'
...
const initials = computed(() => getInitials(employeeName.value))
