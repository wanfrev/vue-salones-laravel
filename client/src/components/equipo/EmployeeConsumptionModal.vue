    ...
    <div v-if="paymentsCtx.showConsumptionModal.value"
      ...
      <div class="w-full max-w-md rounded-2xl border border-border bg-surface p-6 shadow-xl">
        <div class="mb-4">
          ...
          <p class="text-sm text-text-muted">Registra un servicio o producto consumido por el empleado. Se descontará de
            su saldo.</p>
        </div>
        <form class="space-y-4" @submit.prevent="handleSubmit">
          ...
          <div v-if="consumptionBalance" class="rounded-lg bg-bg-secondary p-3 space-y-2">
            <div class="flex items-center justify-between text-sm"><span class="text-text-muted">Tipo de
                pago</span><span class="font-medium text-text">{{ formatPayType(consumptionBalance.payType,
                  consumptionBalance.baseSalary, consumptionBalance.payPercentage) }}</span></div>
            <div class="flex items-center justify-between text-sm"><span class="text-text-muted">Generado en
                servicios</span><span class="font-medium text-success">{{ formatUSD(consumptionBalance.totalEarned)
                }}</span></div>
            <div class="flex items-center justify-between text-sm"><span class="text-text-muted">Pagado +
                Consumido</span><span class="font-medium text-danger">{{ formatUSD(consumptionBalance.totalPaid)
                }}</span></div>
            <div class="flex items-center justify-between border-t border-border pt-2"><span
                ...
                :class="consumptionBalance.pendingBalance > 0 ? 'text-primary' : 'text-text-muted'">{{
                  formatUSD(consumptionBalance.pendingBalance) }}</span></div>
          </div>

          <div><label class="mb-1 block text-sm font-medium text-text">Servicio / Producto consumido</label><input
              v-model="paymentsCtx.consumptionForm.value.concept" type="text"
              class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text outline-none transition-theme focus:border-primary focus:ring-2 focus:ring-primary/30"
              placeholder="Ej: Corte de cabello, Shampoo..." required /></div>
          <div class="grid grid-cols-2 gap-3">
            <div><label class="mb-1 block text-sm font-medium text-text">Monto</label><input
                v-model.number="paymentsCtx.consumptionForm.value.amount" type="number" min="0.01" step="0.01"
                class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text outline-none transition-theme focus:border-primary focus:ring-2 focus:ring-primary/30"
                placeholder="0.00" required /></div>
            <div><label class="mb-1 block text-sm font-medium text-text">Moneda</label><select
                v-model="paymentsCtx.consumptionForm.value.currency"
                class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text outline-none transition-theme focus:border-primary focus:ring-2 focus:ring-primary/30">
          ...
          <div><label class="mb-1 block text-sm font-medium text-text">Fecha</label><input
              v-model="paymentsCtx.consumptionForm.value.date" type="date"
              class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text outline-none transition-theme focus:border-primary focus:ring-2 focus:ring-primary/30"
              required /></div>
          <p v-if="paymentsCtx.consumptionError.value" class="text-sm text-danger">{{ paymentsCtx.consumptionError.value
            }}</p>
          <div class="flex items-center justify-end gap-3">
            ...
            <button type="submit" :disabled="paymentsCtx.consumeMutation.isPending.value"
              class="inline-flex items-center justify-center rounded-lg bg-danger px-4 py-2 text-sm font-semibold text-text-inverse shadow-sm transition-theme hover:bg-danger-hover disabled:cursor-not-allowed disabled:opacity-60">{{
                paymentsCtx.consumeMutation.isPending.value ? 'Guardando...' : 'Debitar consumo' }}</button>
          </div>
        </form>
      </div>
    </div>
