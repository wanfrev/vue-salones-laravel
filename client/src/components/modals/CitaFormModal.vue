    ...
    <form @submit.prevent="handleSubmit" class="space-y-5">
      ...
      <div class="space-y-3">
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          />
          <FormInput v-if="canViewClients" v-model="formData.clientPhone" label="Teléfono" type="tel" placeholder="+58 412 1234567" required
        ...
        <FormInput v-if="canViewClients && !formData.clientId" v-model="formData.clientEmail" label="Correo Electrónico (Opcional)" type="email" placeholder="cliente@correo.com"
...
const isFormValid = computed(() => {
  ...
  const vp = !canViewClients.value || formData.value.clientPhone.trim().length >= 7
...
const confirmButtonLabel = computed(() => {
  ...
  if (canViewClients.value && formData.value.clientPhone.trim().length < 7) return 'Falta teléfono'
...
const handleSubmit = () => {
  ...
  const citaData: CitaFormData & { id?: string; clientPhone?: string; paymentData?: PaymentEditContext } = {
    ...formData.value,
    clientPhone: formData.value.clientPhone || (!canViewClients.value ? '0000000000' : ''),
  }
