...
// Mounted at both /admin/gabinete (a solo doctor logged in as admin — no separate secretary) and
// /dashboard/gabinete (an employee with gabinete_mode) — same component, same data, only the
// record-opening path differs, mirroring how PatientDentalShell derives its own basePath.
...
function openRecord(cita: Cita) {
  ...
  router.push({
    path: `${basePath.value}/clientes/${cita.clientId}/expediente/historia-clinica`,
