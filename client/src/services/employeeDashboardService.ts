...
export const dashboardKeys = {
  ...
  payments: (businessId?: string | null, employeeId?: string | null, branchId?: string | null) => ['employee-payment-history', businessId, employeeId, branchId] as const,
}
...
export const listEmployeePayments = async (businessId: string, employeeId: string, branchId?: string | null) => {
  const params = new URLSearchParams()
  if (branchId) params.set('branch_id', branchId)
  if (employeeId) params.set('employee_id', employeeId)
  const qs = params.toString()
  return await apiRequest<EmployeePaymentHistoryRecord[]>('GET', `/employee-payments?${qs.toString()}`)
}
