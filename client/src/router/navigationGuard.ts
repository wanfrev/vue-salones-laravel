...
export function resolveNavigation(to: NavTarget, ctx: NavContext): string | undefined {
  const resolveHome = () => resolveHomeByRole(ctx.role ?? undefined, ctx.profile?.disable_agenda, ctx.hasFeature('agenda'), ctx.hasFeature('pos'), ctx.hasFeature('servicios'), ctx.hasCapability('staffing.timesheets'), ctx.hasFeature('employees_recibo_only'), !!ctx.profile?.gabinete_mode)
