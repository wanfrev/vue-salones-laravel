...
use App\Services\Dental\EndoAnnexService;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;

class EndoAnnexController
{
    public function __construct(private EndoAnnexService $service)
    {
    }
    ...
    public function store(Request $request, string $clientId): JsonResponse
    {
        ...
        $data = $request->validate(['tooth_number' => ['required', 'integer']]) + $this->validatedSections($request);
        $annex = $this->service->create($clientId, $businessId, $client->branch_id, $data, $request->user()?->id);

        EntityChanged::safe($businessId, 'dental_endo_annex', 'created', $annex->id);
    ...
    public function update(Request $request, string $clientId, string $id): JsonResponse
    {
        ...
        EntityChanged::safe($businessId, 'dental_endo_annex', 'updated', $annex->id);
