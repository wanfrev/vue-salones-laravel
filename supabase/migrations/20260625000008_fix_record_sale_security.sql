-- =============================================================================
-- Fix record_sale: change to security definer so inventory DML bypasses RLS
-- 
-- Bug: record_sale was security invoker. The inventory UPDATE/INSERT inside
-- passed through RLS. If policies on inventory_stock / inventory_movements
-- didn't match, Postgres silently did nothing (0 rows) and returned success.
-- Products sold in POS appeared to succeed but didn't update stock or appear
-- in Finanzas product sales.
-- 
-- Also fixes POSQuickSell which uses sell_product function.
-- =============================================================================
