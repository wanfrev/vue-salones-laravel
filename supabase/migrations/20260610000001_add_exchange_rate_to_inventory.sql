-- Add exchange_rate_used to inventory_movements for historical VES display
-- Without this, product sales VES amounts get recalculated with the current rate,
-- causing historical records to show incorrect values in Venezuela's volatile economy.
alter table public.inventory_movements
  add column if not exists exchange_rate_used numeric(12, 4) not null default 1.0000;

-- Update record_sale to store the exchange rate on inventory movements
...
begin
  ...
  if jsonb_array_length(p_products) > 0 then
    select business_id into v_business_id from public.appointments where id = p_appointment_id;
    ...
    v_rate := coalesce(p_exchange_rate,
      (select ves_exchange_rate from public.businesses where id = v_business_id));
    ...
    loop
      ...
      insert into public.inventory_movements (
        ...
        movement_type, quantity, unit_cost, exchange_rate_used,
